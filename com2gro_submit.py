...
for h,line in enumerate(lines_lig1[2:41]):
            ...
            words = line.split()
            #resname.append(words[0])
            atom.append(words[2])
            #number.append(words[2])
...
for i in onlyfiles[:]:
    ...
    if i[-4:]==".com":
        ...
        for h,line in enumerate(lines[6:45]):
            words = line.split()
            x_cord.append(float(words[1])/10)
            y_cord.append(float(words[2])/10)
            z_cord.append(float(words[3])/10)
        ...
        for j in range(len(x_cord)):
         #   f.write("{:10s}{:5s}{:5s}{:8.3f}{:8.3f}{:8.3f}\n".format(resname[j],atom[j],number[j],x_cord[j],y_cord[j],z_cord[j]))
            ...
            f.write("{:>5s}{:<5.5s}{:>5.5s}{:>5d}{:8.3f}{:8.3f}{:8.3f}\n".format("1","BTV",atom[j],j+1,x_cord[j],y_cord[j],z_cord[j]))
