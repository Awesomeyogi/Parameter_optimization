...
# prep data
log=$input-$cmd-orignl.log

# ---> execute gromacs tasks <---

aprun $APRUN_OPTIONS gmx_mpi grompp -f $cmd.mdp -c $input.gro -p orignal.top -o $input-$cmd-orignl.tpr -maxwarn 1 > $log.1 2>&1   #-n index.ndx 
aprun $APRUN_OPTIONS gmx_mpi mdrun -s $input-$cmd-orignl.tpr -rerun $input.gro -g $input-$cmd-orignl.log
...
itp_file = open("bTVBT4_orignal.itp",'r')
...
aprun $APRUN_OPTIONS gmx_mpi grompp -f $cmd.mdp -c $input.gro -p zero.top -o $input-$cmd-zero.tpr -maxwarn 1 > $log.1 2>&1   #-n index.ndx 
aprun $APRUN_OPTIONS gmx_mpi mdrun -s $input-$cmd-zero.tpr -rerun $input.gro -g $input-$cmd-zero.log 
